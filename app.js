...
`;
  function MaRoutine() {
    ...
    ), vue === "suivi" && /* @__PURE__ */ React.createElement(VueSuivi, { days, habits, todayKey, serie }), vue === "rituels" && /* @__PURE__ */ React.createElement(VueRituels, { habits, setHabits, setDays }))), /* @__PURE__ */ React.createElement("nav", { className: "onglets" }, [["jour", "Ma journ\xE9e"], ["suivi", "Suivi"], ["rituels", "Rituels"]].map(([id, txt]) => /* @__PURE__ */ React.createElement("button", { key: id, className: `onglet ${vue === id ? "on" : ""}`, onClick: () => setVue(id) }, txt))));
  }
  ...
  function VueSuivi({ days, habits, todayKey, serie }) {
    const derniers = useMemo(() => {
      const out = [];
      for (let i = 13; i >= 0; i--) {
        const k = shiftKey(todayKey, -i);
        out.push({ key: k, pct: days[k] ? pctOfDay(days[k], habits) : null, date: fromKey(k) });
      }
      return out;
